...
def Reroteamento(count, path, spectro, holding_time, src, dst, bandwidth, class_type):
    ...
    if topology.has_edge(src, dst) and 'failed' in topology[src][dst] and topology[src][dst]['failed'] == True:
        ...
        #print("capacidade livre",capacity)
            ...
            for sublista in Datapaths:
                ...
                for pathsAble in coluna_7:
                    ...
                    if Contbandwidth == 0:
                        ...
                        # Tabela de requisições aceitas com n° da req, classe ,num_slots, path, posição no spectro, inicio ,holding_time, frac_ht, hops, distancia_km, bandwidth,processo
...
def Quem_falhou_link(pontaa, pontab):
	global topology, ativo, req_accepts, Erlangx
	ativo2 = []
	...
	#with open(f'{env.now}_{pontaa}_{pontab}_{Erlangx}_req_accepts.csv', mode='w', newline='') as file:
	#	writer = csv.writer(file)
	#	for entradax in req_accepts:
	#		writer.writerow(entradax)  # Linhas de dados
	#print("env.now" , env.now, pontaa, pontab)
	#evv= ("env.now" , env.now, pontaa, pontab)
	...
	#print("entradas_filtradas1" , entradas_filtradas)
	#with open(f'{env.now}_{pontaa}_{pontab}_{Erlangx}_entradas_filtradas.csv', mode='w', newline='') as file:
	#	writer = csv.writer(file)
	#	writer.writerow(['env.now', evv])  # Primeira linha com o valor de env.now
	#	for entradax in entradas_filtradas:
	#		writer.writerow(entradax)  # Linhas de dados
	#input()
	numero_de_linhas = len(entradas_filtradas)
	print(f"Quantidade de sol block: {numero_de_linhas}")
	numero_de_linhas = len(req_accepts)
	print(f"Quantidade de sol: {numero_de_linhas}")
	print(f'{env.now}_{pontaa}_{pontab}_{Erlangx}')
