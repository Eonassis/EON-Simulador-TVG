...
def Simulador(self, rate):
		...
		for count in range(1, NUM_OF_REQUESTS + 1):
			...
			#alteracao gustavo Availability
			#sum_ht += holding_time
...
def GerarFalhas():
    global topology, LINK_POINTSF, NODE_POINTSF, Simfim, mensagem_impressa, cont_falhas
    while True:
        ...
        yield env.timeout(0.1)  # Verifica a cada unidade de tempo
        #print("falha gerada")
...
def FalhaNoLink(node1, node2):
	...
	if topology.has_edge(node1, node2):
		...
		ProbabilidadesCairNo(node1)
		ProbabilidadesCairNo(node2)
		reqfalhou = []
...
#funcao calcula probabilidades 
def ProbabilidadesCairNo(node):
	global topology
	origem = node  # O vértice de origem a partir do qual deseja calcular as distâncias.
	distancia_maxima = 2000  # A distância máxima a aplicar probabilidades

	distancias, caminhos = nx.single_source_dijkstra(topology, origem, cutoff=None, weight='weight')

	for destino, caminho in caminhos.items():
		Maxd = distancia_maxima
		passou = 0
		distancias_entre_nos = {}
		for i in range(len(caminho) - 1):
			no_atual = caminho[i]
			proximo_no = caminho[i + 1]
			distancia_aresta = topology[no_atual][proximo_no]['weight']
			distancias_entre_nos[(no_atual, proximo_no)] = distancia_aresta
			Maxd -= distancia_aresta
			if Maxd <= 0:
				passou += 1
	

		if (passou <= 1):
			#print(f"Destino: {destino}, Distância: {distancias[destino]}, Caminho: {caminho}")
			#print("Distâncias entre nós no caminho:", distancias_entre_nos)
			dn = 0
			for u, v in list(distancias_entre_nos):
				dn  += distancias_entre_nos[u,v]
				if dn <= 1000:
					topology[u][v]['fallprob'] = 0.75
				elif dn <= 1500 and dn > 1000:
					topology[u][v]['fallprob'] = 0.15
				elif dn <= 2000 and dn > 1500:
					topology[u][v]['fallprob'] = 0.05
...
def Reroteamento(count, path, spectro, holding_time, src, dst, bandwidth, class_type):
	global topology, req_accepts, ativo, total_req_restauradas, bloqueio_rerroteamento_pr, restauradasF, afetadasF, ativo, afetadas_cascata, afetadas_cascatas, cont_falhas, comp_falhas
	global contador, afetadas_pr, total_req_restauradas_pr_pr, restauradas_pr, nao_restauradas_pr, rr_nao_restauradas_pr, bloqueio_rerroteamento_pr_pr, afetadas_cascatas_vetor
	...
	if count not in desalocateReqRerr:
		afetadasF += 1
		afetadas_log.append(list(filter(lambda x: x[0] == count, req_accepts)))
		#alteracao gustavo Availability
		#sum_ht += holding_time
		conta_falha_tempo(cont_falhas)        
	...
	if topology.has_edge(src, dst) and 'failed' in topology[src][dst] and topology[src][dst]['failed'] == True:
		...
		# Obtém uma nova lista de caminhos após a falha no link
		new_paths = k_paths[src, dst]
		new_paths_N_PATH = new_paths[:N_PATH]
		matriz = [[0, 0] for _ in range(N_PATH)]

		for k in range(N_PATH):
			maior_probabilidade = 0
			for h in range(len(new_paths_N_PATH[k]) - 1):
				u, v = new_paths_N_PATH[k][h], new_paths_N_PATH[k][h + 1]
				probabilidade_aresta = topology[u][v]['fallprob']
				if probabilidade_aresta > maior_probabilidade:
					maior_probabilidade = probabilidade_aresta
			matriz[k][0] = k
			matriz[k][1] = (maior_probabilidade * ALPHA[POSALPHA]) + ((len(new_paths_N_PATH[k])/len(max(new_paths_N_PATH)))*(1-ALPHA[POSALPHA]))
		...
		# mudei rage(N_PATH) para primeira_coluna_ordenada
		ConjSlots = 0
		for i in primeira_coluna_ordenada:
			distance = int(Distance(new_paths[i]))
			num_slots = int(math.ceil(Modulation(distance, bandwidth)))
			check_path = PathIsAble(num_slots, new_paths[i])
			
			if check_path[0] == True:
				# Encontrou um novo caminho, aloque-o
				if count not in desalocateReqRerr:
						#sum_ht += holding_time
						desalocateReqRerr.append(count)
						total_req_restauradas += 1
						restauradasF +=1
						restauradas_log.append(list(filter(lambda x: x[0] == count, req_accepts)))
				FirstFit(count, check_path[1], check_path[2], new_paths[i])
				new_spectro = [check_path[1], check_path[2]]
				processo = env.process(Desalocate(count,new_paths[i],new_spectro,holding_time, env.now))
				# Atualize a tabela de requisições aceitas com as informações do novo caminho
				req_accepts.append([count, class_type, num_slots, new_paths[i], new_spectro, env.now, holding_time, 0, len(new_paths[i]) - 1, distance, bandwidth, processo])
				ativo[count] = 1
				Numsaltos +=  len(new_paths[i])
				Qtd_sol_Numsaltos += 1 
				ConjSlots += 1
		...
		if check_path[0] == False:
			...
			nao_restauradas_log.append(list(filter(lambda x: x[0] == count, req_accepts)))
...
def Desalocate(count, path, spectro, holding_time, inicio):
	global topology, sum_time_up, sum_ht, desalocateReq1, desalocateReq2
	try:
...
# Computa numero de requisicoes bloqueadas por classe
def conta_falha_tempo(numero):
	global afetadas_cascatas_vetor
	if numero not in afetadas_cascatas_vetor:
		afetadas_cascatas_vetor[numero] = 1
	else:
		afetadas_cascatas_vetor[numero] += 1
...
arquivo20  = open('out/'+topologia+'/afetadas_cascata'+'.dat', 'w')
arquivo21  = open('out/'+topologia+'/afetadas_cascatas'+'.dat', 'w')
...
for e in range(ERLANG_MIN, ERLANG_MAX+1, ERLANG_INC):
	...
	global afetadas_log
	global restauradas_log
	global nao_restauradas_log

	afetadas_log = []
	restauradas_log = []
	nao_restauradas_log = []

	intervalo_afetadas_cascatas_v = []                               
	...
	for rep in range(REP):
		...
		Bloqueio_20.append(0)
		Bloqueio_40.append(0)
		Bloqueio_80.append(0)
		Bloqueio_160.append(0)
		Bloqueio_200.append(0)
		Bloqueio_400.append(0)
		...
		afetadas.append(NumReqBlocked+afetadasF)
		...
		afetadas_cascata_v.append(afetadasF)

		print(afetadas_cascatas_vetor)
		print(afetadasF)

		afetadas_cascatas_v.append(afetadas_cascatas_vetor)
	...
	intervalo_afetadas_cascata_v = CalculaIntervalo(afetadas_cascata_v)


	print(afetadas_cascatas_v)
	#for i in range(len(afetadas_cascatas_v)):
	#	intervalo_afetadas_cascatas_v[i] = CalculaIntervalo(afetadas_cascatas_v[i])

	valores_agrupados = {}
	# Itera sobre cada dicionário na lista
	# Encontra todas as chaves únicas em todos os dicionários
	chaves_unicas = set(chave for dicionario in afetadas_cascatas_v for chave in dicionario)

	# Itera sobre cada dicionário na lista
	for dicionario in afetadas_cascatas_v:
		# Itera sobre as chaves únicas
		for chave in chaves_unicas:
			# Adiciona o valor à lista correspondente à chave no dicionário temporário
			valor = dicionario.get(chave, 0)
			if chave not in valores_agrupados:
				valores_agrupados[chave] = [valor]
			else:
				valores_agrupados[chave].append(valor)




	for chave, valores in valores_agrupados.items():
		intervalo_afetadas_cascatas_v.append(CalculaIntervalo(valores))
		print(valores)

	print(intervalo_afetadas_cascatas_v)
	...
	arquivo20.write("\t")
	arquivo20.write(str(intervalo_afetadas_cascata_v[0]))
	arquivo20.write("\t")
	arquivo20.write(str(intervalo_afetadas_cascata_v[0]-intervalo_afetadas_cascata_v[1]))
	arquivo20.write("\t")
	arquivo20.write(str(intervalo_afetadas_cascata_v[0]+intervalo_afetadas_cascata_v[1]))
	arquivo20.write("\n")
	...
	#for i in range(len(afetadas_cascatas_v)):
	#	#saltos
	#	arquivo21.write(str(i+1))
	#	arquivo21.write("\t")
	#	arquivo21.write(str(e))
	#	arquivo21.write("\t")
	#	arquivo21.write(str(intervalo_afetadas_cascatas_v[i][0]))
	#	arquivo21.write("\t")
	#	arquivo21.write(str(intervalo_afetadas_cascatas_v[i][0]-intervalo_afetadas_cascatas_v[i][1]))
	#	arquivo21.write("\t")
	#	arquivo21.write(str(intervalo_afetadas_cascatas_v[i][0]+intervalo_afetadas_cascatas_v[i][1]))
	#	arquivo21.write("\n")
...
################grava csv###################
#						req, classe ,num_slots, path, posição no spectro, inicio ,holding_time, frac_ht, hops, distancia_km, bandwidth,processo
header = ['req', 'classe' ,'num_slots', 'path', 'posicao_spectro', 'inicio' ,'holding_time', 'frac_ht', 'hops', 'distancia', 'bandwidth', 'processo' ]

with open('out/'+topologia+'/afetadas.csv', 'w') as f:
	writer = csv.writer(f)
	writer.writerow(header)
	writer.writerows(afetadas_log)

with open('out/'+topologia+'/restauradas.csv', 'w') as f:
	writer = csv.writer(f)
	writer.writerow(header)
	writer.writerows(restauradas_log)

with open('out/'+topologia+'/nao_restauradas.csv', 'w') as f:
	writer = csv.writer(f)
	writer.writerow(header)
	writer.writerows(nao_restauradas_log)
